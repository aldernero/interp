...
func TestCubicSpline(t *testing.T) {
	...
	assert.Equal(t, 1, cs.intervalFunc(1))
	assert.Equal(t, 1, cs.Interval(1.1))
...
func TestRegularCubicSpline(t *testing.T) {
	x := []float64{0, 1, 2, 3, 4}
	y := []float64{0, 0.2, 0.1, 0.45, 0.3}
	cs, err := NewRegularCubicSpline(x, y)
	assert.NoError(t, err)
	assert.Equal(t, 1, cs.intervalFunc(1))
	assert.Equal(t, 1, cs.intervalFunc(1.1))
	assert.Equal(t, 0.0, cs.Eval(-1))
	assert.Equal(t, 0.3, cs.Eval(10))
	for i, val := range x {
		assert.Equal(t, y[i], cs.Eval(val))
